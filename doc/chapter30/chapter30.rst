...
我们使用基本定时器循环定时0.5s并使能定时器中断，每到0.5s就在定时器中断服务函数翻转LED灯，使得最终效果LED灯暗0.5s，亮0.5s，如此循环。

硬件设计
''''''''''''''

基本定时器没有相关GPIO，这里我们只用定时器的定时功能，无效其他外部引脚，至于LED灯硬件可参考GPIO章节。
...
实验中先初始化系统时钟，用到LED灯，需要对其初始化配置。LED_GPIO_Config函数是定义在bsp_led.c文件的完成LED灯GPIO初始化配置的程序。
...
保证开发板相关硬件连接正确，把编译好的程序下载到开发板。开始LED灯是暗的，等一会LED灯变为红色，再等一会又暗了，如此反复。如果我们使用表钟与LED灯闪烁对比，可以发现它是每0.5s改变一次LED灯状态的。
